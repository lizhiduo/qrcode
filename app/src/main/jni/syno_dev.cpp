...
int set_speed(speed_t speed, unsigned char waitTime){

    struct termios cfg;

    if(tcgetattr(syno_fd,&cfg)!=0){
    ...
    //set time
    cfg.c_cc[VTIME]= waitTime; // n*100ms
    cfg.c_cc[VMIN] = 0;

    if(tcsetattr(syno_fd, TCSANOW, &cfg) != 0){
